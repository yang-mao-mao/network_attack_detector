...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        if hasattr(packet_win, "demo_requested"):
            packet_win.demo_requested.connect(self.load_demo_data)
    ...
    def load_demo_data(self) -> None:
        """Load demo traffic into the integrated UI pages."""
        from src.demo_data import build_demo_dataset, filter_result_by_rule_prefix

        dataset = build_demo_dataset()

        behavior_win = self._stack.widget(0)
        feature_win = self._stack.widget(1)
        packet_win = self._stack.widget(2)
        stat_window = self._stack.widget(4)

        if hasattr(packet_win, "clear_packets"):
            packet_win.clear_packets()
        self._clear_result_panel(feature_win)
        self._clear_result_panel(behavior_win)
        if hasattr(stat_window, "clear_data"):
            stat_window.clear_data()

        for item in dataset.items:
            if hasattr(packet_win, "add_packet"):
                packet_win.add_packet(item.packet)

            feature_result = filter_result_by_rule_prefix(
                item.result,
                "SIG-",
                "SignatureEngine",
            )
            behavior_result = filter_result_by_rule_prefix(
                item.result,
                "BEH-",
                "BehaviorEngine",
            )
            self._add_result_to_panel(feature_win, feature_result)
            self._add_result_to_panel(behavior_win, behavior_result)

            if hasattr(stat_window, "add_alert"):
                for alert in item.result.alerts:
                    stat_window.add_alert(alert)

        if hasattr(packet_win, "show_demo_loaded"):
            packet_win.show_demo_loaded(len(dataset.packets), len(dataset.alerts))
        self._switch_to(2)

    @staticmethod
    def _clear_result_panel(page: QWidget) -> None:
        panel = getattr(page, "_result_panel", None)
        if panel is not None and hasattr(panel, "clear_results"):
            panel.clear_results()

    @staticmethod
    def _add_result_to_panel(
        page: QWidget,
        result: object | None,
    ) -> None:
        if result is None:
            return
        panel = getattr(page, "_result_panel", None)
        if panel is not None and hasattr(panel, "add_result"):
            panel.add_result(result)


# =============================================================================
# main
# =============================================================================
def main(load_demo: bool = False) -> None:
    ...
    window.show()
    if load_demo:
        QTimer.singleShot(0, window.load_demo_data)
    sys.exit(app.exec())
