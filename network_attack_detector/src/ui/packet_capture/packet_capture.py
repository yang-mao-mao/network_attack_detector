...
from src.capture.live_capture import CaptureState, LiveCapture
...
class PacketCaptureWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._capture_display_name: str | None = None
        ...
        self.setStyleSheet(CSS_APP_GLOBAL.format(
    ...
    def _build_menu_bar(self) -> QMenuBar:
        ...
        load_demo_action.triggered.connect(self._on_load_demo_data)
    ...
    def _on_load_demo_data(self) -> None:
        """Ask the application shell to inject reproducible demo traffic."""
        try:
            self._status_bar.showMessage("Loading demo data...")
        except Exception:
            pass
        self.demo_requested.emit()

    def show_demo_loaded(self, packet_count: int, alert_count: int) -> None:
        """Show the result of a demo-data load in the page status bar."""
        self._status_bar.showMessage(
            f"Demo data loaded: {packet_count} packets, {alert_count} alerts"
        )
    ...
    def _populate_interfaces(self) -> None:
        ...
        showing_all_interfaces = False
        try:
            interfaces = CaptureInterface.get_interfaces(only_up=True)
            if not interfaces:
                interfaces = CaptureInterface.get_interfaces(only_up=False)
                showing_all_interfaces = bool(interfaces)
        except Exception:
            # Fallback: provide at least the loopback
            interfaces = []

        if not interfaces:
            no_iface = QAction("(no interfaces found - check psutil/scapy/Npcap)", self)
            no_iface.setEnabled(False)
            self._start_menu.addAction(no_iface)
            self._status_bar.showMessage(
                "No interfaces found. Check Python environment, psutil, Scapy, and Npcap."
            )
            return

        if showing_all_interfaces:
            hint = QAction("(no active interface found - showing all detected interfaces)", self)
            hint.setEnabled(False)
            self._start_menu.addAction(hint)
            self._start_menu.addSeparator()
            self._status_bar.showMessage(
                "No active interface found. Showing all detected interfaces."
            )

        for iface in interfaces:
            display_name = iface.friendly_name or iface.name
            label = display_name
            if iface.ip_addresses:
                label += f"  ({', '.join(iface.ip_addresses)})"
            if display_name != iface.name and not iface.name.startswith("\\Device\\NPF"):
                label += f"  [{iface.name}]"
            if not iface.is_up:
                label += "  [DOWN]"
            action = QAction(label, self)
            action.setStatusTip(iface.name)
            action.triggered.connect(
                lambda _checked, dev=iface.name, title=display_name: (
                    self._on_start_capturing(dev, title)
                )
            )
            self._start_menu.addAction(action)

    def _on_start_capturing(self, device: str, display_name: str | None = None) -> None:
        ...
        self._capture_display_name = display_name or device
    ...
    def _on_stop_capturing(self) -> None:
        ...
        self._capture_display_name = None
    ...
    def _show_no_traffic_placeholder(self) -> None:
        ...
        device = self._capture_display_name or self._capture_device or "unknown"
